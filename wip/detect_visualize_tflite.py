...
def annotate_objects(image, results, labels):
    ...
    for obj in results:
        # Convert the bounding box figures from relative coordinates
        # to absolute coordinates based on the image resolution
        ymin, xmin, ymax, xmax = obj['bounding_box']
        ...
        text = f"{labels[obj['class_id']]}: {obj['score']:.2f}"
...
def main():
    ...
    annotated_image = annotate_objects(image, results, labels)
    annotated_image.save("annotated_image.jpg")

    print('Processing & visualization time: %.1fms' % elapsed_ms)
